...
async fn cram_rss_to_ast_export() {
    ...
    .await
    .expect("Failed to convert RSS to AST");
...
async fn cram_ast_to_epub_conversion() {
    ...
    let mut document = Document::new("Cram EPUB Test".to_string(), "Cram Author".to_string());
...
<rss version="2.0">
    ...
    assert!(
        has_heading || has_list,
        "AST should contain structured content"
    );
    ...
    println!(
        "  Pipeline: RSS -> AST ({} bytes) -> EPUB ({} bytes)",
        ast_json.len(),
        epub_size
    );
    println!(
        "  Content blocks: paragraph={}, structured={}",
        has_paragraph,
        has_heading || has_list
    );
}
...
<rss version="2.0">
    ...
    document_to_epub(&document, malformed_epub_path.to_str().unwrap())
        .await
        .unwrap();
...
async fn cram_rss_to_markdown_conversion() {
    ...
    // Document structure
    assert!(
        markdown_content.contains("# Cram Markdown Document"),
        "Should have document title as H1"
    );
    assert!(
        markdown_content.contains("**Author:** Cram Markdown Author"),
        "Should include author metadata"
    );
    assert!(
        markdown_content.contains("**Generated:**"),
        "Should include generation timestamp"
    );
    assert!(
        markdown_content.contains("**Total Articles:** 2"),
        "Should show correct article count"
    );
    ...
    assert!(
        markdown_content.contains("- [Cram Markdown Feed]"),
        "Should list feed in TOC"
    );
    assert!(
        markdown_content.contains("  - [Cram Test Article 1]"),
        "Should list articles in TOC"
    );
    assert!(
        markdown_content.contains("  - [Cram Test Article 2]"),
        "Should list all articles in TOC"
    );

    // Feed structure
    assert!(
        markdown_content.contains("## Cram Markdown Feed"),
        "Should have feed name as H2"
    );
    assert!(
        markdown_content.contains("A feed for cram testing"),
        "Should include feed description"
    );

    // Article structure
    assert!(
        markdown_content.contains("### Cram Test Article 1"),
        "Should have article titles as H3"
    );
    assert!(
        markdown_content.contains("### Cram Test Article 2"),
        "Should have all article titles"
    );
    assert!(
        markdown_content.contains("**Source:** Cram Markdown Feed"),
        "Should include feed source"
    );
    assert!(
        markdown_content.contains("**Link:** [Read original article]"),
        "Should include article links"
    );

    // Content formatting
    assert!(
        markdown_content.contains("**test content**"),
        "Should preserve bold formatting from HTML"
    );
    assert!(
        markdown_content.contains("*formatting*"),
        "Should preserve italic formatting from HTML"
    );
    assert!(
        markdown_content.contains("- Item 1"),
        "Should convert HTML lists to Markdown"
    );
    assert!(
        markdown_content.contains("- Item 2"),
        "Should preserve list structure"
    );
    assert!(
        markdown_content.contains("#### Heading"),
        "Should convert HTML headings"
    );
    assert!(
        markdown_content.contains("`code`"),
        "Should convert HTML code elements"
    );

    // Structure separators
    assert!(
        markdown_content.contains("---"),
        "Should include section separators"
    );
...
async fn cram_ast_to_markdown_comprehensive() {
    ...
    article.content = vec![
        ...
        // Quote block
        daily_feed::ast::ContentBlock::Quote(daily_feed::ast::TextContent::from_spans(vec![
    ...
    let comment = daily_feed::ast::Comment {
        ...
        content: vec![
            daily_feed::ast::ContentBlock::Paragraph(daily_feed::ast::TextContent::from_spans(
                vec![
                    ...
                    daily_feed::ast::TextSpan::link(
                        "link".to_string(),
                        "https://comment.example.com".to_string(),
                    ),
                ],
            )),
            daily_feed::ast::ContentBlock::List {
    ...
    assert!(
        metadata.len() > 1000,
        "Comprehensive markdown should be substantial"
    );
    ...
    assert!(markdown_content
        .contains("[Comprehensive Example Link](https://comprehensive.example.com)"));
    ...
    assert!(markdown_content
        .contains("> This comment has **bold text** and a [link](https://comment.example.com)"));
...
<rss version="2.0">
    ...
    document_to_output(
        &document,
        empty_markdown_path.to_str().unwrap(),
        &OutputFormat::Markdown,
    )
    .await
    .unwrap();
    ...
    document_to_output(
        &minimal_document,
        minimal_markdown_path.to_str().unwrap(),
        &OutputFormat::Markdown,
    )
    .await
    .unwrap();
    ...
    document_to_output(
        &special_document,
        special_markdown_path.to_str().unwrap(),
        &OutputFormat::Markdown,
    )
    .await
    .unwrap();
...
<rss version="2.0">
    ...
    document_to_output(
        &loaded_document,
        roundtrip_markdown_path.to_str().unwrap(),
        &OutputFormat::Markdown,
    )
    .await
    .unwrap();
    ...
    assert!(
        markdown_size > 300,
        "CLI markdown output should be substantial"
    );
